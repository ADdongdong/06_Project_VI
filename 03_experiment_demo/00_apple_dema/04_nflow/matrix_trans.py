...
# 这个array中，有多个self.size*self.size的二维array
# 所以，初始化对象的时候，可以传入一个(n, self.size, self.size)shape的数据


class matrix_data():
    def __init__(self, data, size) -> None:
        self.data = data  # 传入打分矩阵列表
        self.size = size  # 传入每个矩阵的大小

    # private:输入一个self.size*self.size的array,对这个self.size*self.size的array进行处理
    ...
    def get_result_matrix_npy(self, filename: str) -> np.array:
        ...
        for i in self.data:
            i = i.astype(float)
            i += 1.0  # 个矩阵的所有元素+1 打分的范围从1-self.size
    ...
    # 将多个self.size*self.size的array保存在xlsx表格中
    ...
    def to_normalizing_flow_data(self, data) -> list:
        # 对于每一个矩阵，计算其集几何平均值：将self.size个元素连续乘，然后对结果开self.size次方
        result = []
        for matrix in data:
            x = []  # 这个数组用来记录每一个矩阵的self.size个几何平均值
            for i in matrix:  # 一个matrix里面有self.size行，就会计算出来self.size个元素
                num = 1
                for j in range(self.size):
                    num = i[j]*num
                # 对num开n次方
