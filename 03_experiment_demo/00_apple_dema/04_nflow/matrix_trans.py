...
# 输入一个8*8的array,对这个8*8的array进行处理
...
def result_matrix(List, filename: str) -> np.array:
    result = []
    # print(list[0])
...
def save_excel(result: np.array, file_name: str) -> None:
    # 创建一个工作簿
    wb = Workbook()
    # 创建一个工作表
    ws = wb.active
...
def normalizing_data(data) -> list:
